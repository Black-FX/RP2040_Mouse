...
bool timer1_callback(struct repeating_timer *t)
{
  ...
  if (mouseDistanceX > 0) {
    ...
    if (mouseEncoderPhaseX == 0) gpio_put(XA_PIN, 1); // Set X1 to 1
    if (mouseEncoderPhaseX == 1) gpio_put(XB_PIN, 1); // Set X2 to 1
    if (mouseEncoderPhaseX == 2) gpio_put(XA_PIN, 0); // Set X1 to 0
    if (mouseEncoderPhaseX == 3) gpio_put(XB_PIN, 0); // Set X2 to 0
...
bool timer2_callback(struct repeating_timer *t)
{
  ...
  if (mouseDistanceY > 0) {
    ...
    // Set the output pins according to the current phase
    if (mouseEncoderPhaseY == 3) gpio_put(YA_PIN, 0); // Set Y1 to 0
    if (mouseEncoderPhaseY == 2) gpio_put(YB_PIN, 0); // Set Y2 to 0
    if (mouseEncoderPhaseY == 1) gpio_put(YA_PIN, 1); // Set Y1 to 1
    if (mouseEncoderPhaseY == 0) gpio_put(YB_PIN, 1); // Set Y2 to 1
